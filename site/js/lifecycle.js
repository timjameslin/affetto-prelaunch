...
function StageCompressedTimeline() {
  ...
  // Layout: labels sit above bars with proper vertical gap. Previously the
  // labels (TRAD_Y-4) sat too close to the top of each bar (only 4px gap),
  // making them feel crammed. Increased gap to 12px for visual breathing room.
  var W = 480,
    H = 200;
  var PAD_L = 16,
    PAD_R = 24,
  ...
  var AFF_Y = 120;
  var BAR_H = 18;
  var LABEL_GAP = 12; // distance from label baseline to bar top
...
function StageGridDiagram() {
  ...
  //
  // Layout: labels (01-09) sit ABOVE each cell rectangle, not inside.
  // Previously labels overlapped the top border of each cell. Reserved
  // header space for labels; cells start below.
  var W = 480,
    H = 220;
  var COLS = 3,
    ROWS = 3;
  var PAD_X = 32;
  var PAD_TOP = 24;
  var PAD_BOTTOM = 36; // room for "NINE-LOCATION EXAMPLE — ALL LIVE" caption
  var LABEL_HEIGHT = 14; // vertical room for "01"-"09" labels
  var cellW = (W - PAD_X * 2) / COLS;
  var cellH = (H - PAD_TOP - PAD_BOTTOM - ROWS * LABEL_HEIGHT) / ROWS;
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    var x = PAD_X + cell.c * cellW;
    // Each row gets LABEL_HEIGHT of header space + cellH of cell space
    var rowY = PAD_TOP + cell.r * (cellH + LABEL_HEIGHT);
    var labelY = rowY + 10; // baseline for the "01"-"09" label
    var cellY = rowY + LABEL_HEIGHT; // cell rectangle top
    ...
    }, "0", cell.idx + 1), /*#__PURE__*/React.createElement("rect", {
      x: x + 4,
      y: cellY,
      width: cellW - 8,
      height: cellH,
